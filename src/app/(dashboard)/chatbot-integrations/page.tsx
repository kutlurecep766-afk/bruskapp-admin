...
const PLATFORM_SVGS: Record<string, string> = {
  ...
  trendyol: '<svg viewBox="0 0 48 48"><rect width="48" height="48" rx="12" fill="#F27A00"/><text x="24" y="33" font-family="Arial,sans-serif" font-weight="900" font-size="30" text-anchor="middle" fill="white">t</text></svg>',
  hepsiburada: '<svg viewBox="0 0 48 48"><rect width="48" height="48" rx="12" fill="#572B85"/><rect x="10" y="12" width="28" height="24" rx="3" fill="white"/><text x="16" y="29" font-family="Arial,sans-serif" font-weight="800" font-size="14" fill="#572B85">HB</text></svg>',
  n11: '<svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="24" fill="#D7003A"/><text x="15" y="32" font-family="Arial,sans-serif" font-weight="900" font-size="18" fill="white">n</text><text x="26" y="32" font-family="Arial,sans-serif" font-weight="700" font-size="13" fill="white">11</text></svg>',
...
const platforms = [
  { key: 'whatsapp', label: 'WhatsApp', color: 'text-green-400', bg: 'bg-[#25D366]/10', border: 'border-green-500/20', note: "WhatsApp Business hesabÄ±nÄ±zÄ± baÄŸlayÄ±n", type: 'zernio' as const },
  { key: 'instagram', label: 'Instagram', color: 'text-pink-400', bg: 'bg-[#E4405F]/10', border: 'border-pink-500/20', note: "Instagram iÅŸletme hesabÄ±nÄ±zÄ± baÄŸlayÄ±n", type: 'zernio' as const },
  { key: 'facebook', label: 'Facebook Messenger', color: 'text-blue-400', bg: 'bg-[#0866FF]/10', border: 'border-blue-500/20', note: "Facebook sayfanÄ±zÄ± baÄŸlayÄ±n", type: 'zernio' as const },
  { key: 'telegram', label: 'Telegram', color: 'text-sky-400', bg: 'bg-[#0088CC]/10', border: 'border-sky-500/20', note: "BotFather'da oluÅŸturduÄŸunuz botu baÄŸlayÄ±n", type: 'telegram' as const },
  { key: 'webchat', label: 'Web Chat', color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', note: "Web sitenize entegre edin, mÃ¼ÅŸterilerinizle canlÄ± iletiÅŸim kurun", type: 'webchat' as const },
  { key: 'trendyol', label: 'Trendyol', color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/20', note: "Trendyol maÄŸazanÄ±za gelen sorularÄ± cevaplayÄ±n", type: 'apikey' as const },
  { key: 'hepsiburada', label: 'Hepsiburada', color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20', note: "Hepsiburada maÄŸazanÄ±za gelen sorularÄ± cevaplayÄ±n", type: 'apikey' as const },
  { key: 'n11', label: 'n11', color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20', note: "n11 maÄŸazanÄ±za gelen sorularÄ± cevaplayÄ±n", type: 'apikey' as const },
]

const WEB_CHAT_STEPS = [
  { title: 'Kodu KopyalayÄ±n', desc: 'AÅŸaÄŸÄ±daki kodu kopyalayÄ±n, web sitenize ekleyeceksiniz.' },
  { title: 'Web Sitenize Ekleyin', desc: 'WordPress, Wix, Ä°kas vb. panelinizde "Ã–zel Kod" veya "Header" bÃ¶lÃ¼mÃ¼ne yapÄ±ÅŸtÄ±rÄ±n.' },
  { title: 'Tamam!', desc: 'MÃ¼ÅŸterileriniz web sitenizde sohbet balonu gÃ¶recek. Gelen mesajlar bu panele dÃ¼ÅŸecek.' },
]
...
export default function ChatbotIntegrationsPage() {
  ...
  useEffect(() => {
    ...
    if (connected) showToast('success', connected + ' baÅŸarÄ±yla baÄŸlandÄ±!')
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        if (isApp) {
          ...
          setConnecting(null)
          showToast('success', platform.charAt(0).toUpperCase() + platform.slice(1) + ' sayfasÄ± aÃ§Ä±ldÄ±, onaylayÄ±n ve geri dÃ¶nÃ¼n.')
        } else {
          window.location.href = data.url
        }
      } else {
        showToast('error', 'BaÄŸlantÄ± URL alÄ±namadÄ±: ' + (data.message || ''))
        setConnecting(null)
      }
    } catch (e: any) {
      clearTimeout(timeout)
      showToast('error', 'BaÄŸlantÄ± hatasÄ±: ' + (e.message || ''))
      setConnecting(null)
    }
  }
  ...
  const handleApiKeySave = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setApiKeyModal(null)
        showToast('success', apiKeyModal.label + ' baÄŸlandÄ±!')
      } else {
        setApiKeyError(json.message || 'BaÄŸlantÄ± baÅŸarÄ±sÄ±z')
      }
    } catch {
      setApiKeyError('BaÄŸlantÄ± hatasÄ±')
    } finally {
  ...
  const handleApiKeyDisconnect = async (platform: string) => {
    try {
      ...
      showToast('success', platform + ' baÄŸlantÄ±sÄ± kesildi')
    } catch {}
  }

  const handleTelegramConnect = async () => {
    if (!telegramToken.trim()) {
      setTelegramError('LÃ¼tfen bot token girin')
      return
    }
    ...
    try {
      const tenantId = currentTenantId || await getTenantId()
      if (!tenantId) { setTelegramError('Tenant bulunamadÄ±'); setTelegramTesting(false); return }
      ...
      if (json.success) {
        ...
        showToast('success', 'Telegram botu baÅŸarÄ±yla baÄŸlandÄ±!')
      } else {
        setTelegramError(json.message || 'BaÄŸlantÄ± baÅŸarÄ±sÄ±z')
      }
    } catch { setTelegramError('BaÄŸlantÄ± hatasÄ±') } finally {
  ...
  const handleTelegramDisconnect = async () => {
    ...
    try {
      ...
      showToast('success', 'Telegram baÄŸlantÄ±sÄ± kesildi')
    } catch {}
  }
  ...
  const handleDisconnect = async (platform: string, type: string) => {
    ...
    if (type === 'webchat') { setWebchatConnected(false); showToast('success', 'Web Chat devre dÄ±ÅŸÄ± bÄ±rakÄ±ldÄ±'); return }
    try {
      const tenantId = currentTenantId || await getTenantId()
      if (!tenantId) return
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="bg-[#0d1117] border border-[#1a2332] rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-semibold">Telegram Bot BaÄŸla</h3>
              </div>
            ...
            <p className="text-sm text-gray-500 mb-4">@BotFather ile yeni bir bot oluÅŸturun, aldÄ±ÄŸÄ±nÄ±z tokenÄ± aÅŸaÄŸÄ±ya yapÄ±ÅŸtÄ±rÄ±n.</p>
            ...
            <button onClick={handleTelegramConnect} disabled={telegramTesting} className="w-full py-3 rounded-xl text-sm font-medium bg-gradient-to-r from-sky-600 to-sky-500 text-white hover:shadow-lg hover:shadow-sky-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {telegramTesting ? <><Loader2 className="w-4 h-4 animate-spin" /> DoÄŸrulanÄ±yor...</> : <><Bot className="w-4 h-4" /> DoÄŸrula ve BaÄŸla</>}
            </button>
          </div>
        </div>
      )}
              ...
              <div className="bg-[#080b12] border border-[#1a2332] rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => copyToClipboard(webchatCode)} className="flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 transition-colors">
                    {copied ? <><Check size={12} className="text-green-400" /> KopyalandÄ±</> : <><Copy size={12} /> Kopyala</>}
                  </button>
                </div>
                <code className="block text-xs text-gray-300 font-mono bg-[#0d1117] rounded-lg p-3 border border-[#1a2332] break-all">{webchatCode || 'YÃ¼kleniyor...'}</code>
              </div>
          ...
          <div className="bg-[#0d1117] border border-[#1a2332] rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-semibold">{apiKeyModal.label} API BaÄŸla</h3>
              </div>
            ...
            <p className="text-sm text-gray-500 mb-4">{apiKeyModal.label} satÄ±cÄ± panelinizden API anahtarlarÄ±nÄ±zÄ± alÄ±p girin. Gelen mÃ¼ÅŸteri sorularÄ± otomatik cevaplanacak.</p>
            ...
            <button onClick={handleApiKeySave} disabled={apiKeySaving || !apiKeyForm.apiKey || !apiKeyForm.apiSecret} className="w-full mt-4 py-3 rounded-xl text-sm font-medium bg-gradient-to-r from-blue-600 to-blue-500 text-white hover:shadow-lg hover:shadow-blue-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {apiKeySaving ? <><Loader2 className="w-4 h-4 animate-spin" /> DoÄŸrulanÄ±yor...</> : <><Link2 className="w-4 h-4" /> BaÄŸla</>}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Chatbot EntegrasyonlarÄ±</h1>
          <p className="text-sm text-gray-500 mt-1">TÃ¼m platformlarÄ± baÄŸlayÄ±n, gelen sorular otomatik cevap alsÄ±n</p>
        </div>
      </div>
            ...
            <div key={p.key} className={'bg-[#0d1117]/80 backdrop-blur-xl border rounded-2xl p-6 transition-all ' + (isConnected ? 'border-green-500/20' : 'border-[#1a2332] hover:border-blue-500/30')}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {isConnected
                        ? (p.key === 'telegram' && telegramBotInfo ? '@' + (telegramBotInfo.username || '') : 'BaÄŸlÄ±')
                        : p.note}
                    </p>
                  </div>
                </div>
              ...
              {isConnected ? (
                ...
                <button onClick={() => handleConnect(p.key, p.type)} disabled={connecting === p.key} className="w-full py-2.5 rounded-xl text-sm font-medium bg-gradient-to-r from-blue-600 to-blue-500 text-white hover:shadow-lg hover:shadow-blue-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {connecting === p.key ? <><Loader2 className="w-4 h-4 animate-spin" /> BaÄŸlanÄ±yor...</> : <><Link2 className="w-4 h-4" /> Hesap BaÄŸla</>}
                </button>
              )}
            </div>
          )
