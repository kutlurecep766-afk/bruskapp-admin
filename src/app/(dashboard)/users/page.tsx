...
export default function UsersPage() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSuccess('Isletme olusturuldu! Slug: ' + d.slug)
        setBusinessName(''); setEmail(''); setPassword('')
        return
      }
  ...
  return (
    <div className="max-w-lg mx-auto mt-10">
      <div className="bg-[#0d1117]/80 backdrop-blur-xl border border-[#1a2332] rounded-2xl p-8 shadow-2xl">
        ...
        <h2 className="text-lg font-semibold text-white mb-2 text-center">Yeni Isletme Olustur</h2>
        <p className="text-sm text-gray-500 mb-6 text-center">Yeni bir isletme hesabi acin</p>
        <div className="space-y-4">
          ...
          <button onClick={handleRegister} disabled={loading || !businessName || !email || !password} className="w-full py-2.5 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-xl text-sm font-medium hover:shadow-lg hover:shadow-blue-500/25 transition-all disabled:opacity-50">{loading ? 'Olusturuluyor...' : 'Isletme Olustur'}</button>
        </div>
      </div>
    </div>
  )
